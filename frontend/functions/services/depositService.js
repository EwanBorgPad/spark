...
const createUserDeposit = async ({ db, amount, projectId, walletAddress, lbpAddress, tokenAddress, txId, tierId, nftAddress, json }) => {
...
const getUsersDepositedAmount = async ({ db, projectId, walletAddress }) => {
    const results = (await db
        .run(sql `SELECT amount_deposited FROM deposit WHERE from_address = ${walletAddress} AND project_id = ${projectId};`)).results;
...
const getDepositStatus = async ({ db, projectId, walletAddress, rpcUrl }) => {
    ...
    if (!userMaxCap)
        throw new Error(`Misconfigured project (${projectId}), (userMaxCap) field is missing`);
    if (!userMinCap)
        throw new Error(`Misconfigured project (${projectId}), (userMinCap) field is missing`);
    ...
    if (!decimals)
        throw new Error('Number of decimals is not defined!');
    // Math logic below
    ...
    if (!startTime)
        throw new Error(`Misconfigured project (${projectId}), (startDate) field is missing`);
    return {
        amountDeposited,
        minAmountAllowed,
        maxAmountAllowed,
...
const limitDecimals = (num, decimals) => {
