 ...
 * @param rpcUrl
 */
const getEligibilityStatus = async ({ db, address, projectId, rpcUrl }) => {
    ...
    const project = await db
        ...
        .get();
    const compliancesWithCompletion = [];
    ...
    const tiersWithCompletion = [];
    if (!project)
        throw new Error(`EligibilityService: Project (id=?) not found!`);
    for (const tier of project.json.info.tiers) {
        const tierQuestsWithCompletion = [];
    ...
    const whitelist = await db
        .select()
        .from(whitelistTable)
        .where(and(eq(whitelistTable.address, address), eq(whitelistTable.projectId, projectId)))
        .get();
