...
export const onRequestPost: PagesFunction<ENV> = async (ctx) => {
  ...
  try {
    ...
    const userTotalDepositAmount: number = await DepositService.getUsersDepositedAmount({
      db,
      projectId,
      walletAddress,
    })
    ...
    const projectTokenLimit = Number(project.info.raisedTokenMaxCap)
    ...
    const minInvestmentPerUser = Number(eligibilityTier.benefits.minInvestment)
    const maxInvestmentPerUser = Number(eligibilityTier.benefits.maxInvestment)
    ...
    const totalAmount = amount + userTotalDepositAmount
