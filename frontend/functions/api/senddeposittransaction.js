...
export const onRequestPost = async (ctx) => {
    ...
    try {
        ...
        if (!data?.serializedTx)
            return jsonResponse({ error: 'Request error. Missing serializedTx' }, 404);
        if (!projectId)
            return jsonResponse({ error: 'Request error. ProjectId serializedTx' }, 404);
        if (error)
            return jsonResponse({ error: 'Request error!' }, 404);
        ...
        const { amountInLamports, decimals, lbpAddress, nftAddress, tokenAddress, tokenAmount, userWalletAddress } = await extractTransactionData(txId, heliusApiKey, cluster);
        ...
        // }
        // update db
        if (transactionStatus.confirmationStatus &&
...
async function extractTransactionData(txId, heliusApiKey, cluster) {
